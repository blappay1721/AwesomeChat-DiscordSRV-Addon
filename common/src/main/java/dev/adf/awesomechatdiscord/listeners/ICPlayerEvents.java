...
public class ICPlayerEvents implements Listener {
    ...
    static {
        Scheduler.runTaskTimerAsynchronously(IC.plugin, () -> CACHED_PROPERTIES.cleanUp(), 12000, 12000);
    }
    ...
    private void populate(OfflineICPlayer player, boolean scheduleAsync) {
        if (scheduleAsync) {
            Scheduler.runTaskAsynchronously(AwesomeChatDiscordAddon.plugin, () -> populate(player, false));
            return;
        }
