...
public class BannerGraphics {
    ...
    public static BannerAssetResult generateBannerAssets(ItemStack item) {
        BufferedImage baseImage = AwesomeChatDiscordAddon.plugin.getResourceManager().getTextureManager().getTexture(ResourceRegistry.ENTITY_TEXTURE_LOCATION + "banner_base").getTexture(64, 64);
        ...
        for (Pattern pattern : patterns) {
            ...
            BufferedImage image = AwesomeChatDiscordAddon.plugin.getResourceManager().getTextureManager().getTexture(ResourceRegistry.BANNER_TEXTURE_LOCATION + typeKey.value()).getTexture(64, 64);
    ...
    public static BannerAssetResult generateShieldAssets(ItemStack item) {
        ...
        BufferedImage baseImage = AwesomeChatDiscordAddon.plugin.getResourceManager().getTextureManager().getTexture(ResourceRegistry.ENTITY_TEXTURE_LOCATION + "shield_base").getTexture(64, 64);
        BufferedImage patternsImage = new BufferedImage(64, 64, BufferedImage.TYPE_INT_ARGB);
        ...
        for (Pattern pattern : patterns) {
            ...
            Color color = new Color(pattern.getColor().getColor().asRGB());
            Key typeKey = NMSAddon.getInstance().getPatternTypeKey(type);
            BufferedImage image = AwesomeChatDiscordAddon.plugin.getResourceManager().getTextureManager().getTexture(ResourceRegistry.SHIELD_TEXTURE_LOCATION + typeKey.value()).getTexture(64, 64);
            BufferedImage tint = new BufferedImage(64, 64, BufferedImage.TYPE_INT_ARGB);
            Graphics2D g3 = tint.createGraphics();
            g3.setColor(color);
            g3.fillRect(0, 0, 64, 64);
    ...
    private static BannerAssetResult getDefaultShieldAssets() {
        return new BannerAssetResult(AwesomeChatDiscordAddon.plugin.getResourceManager().getTextureManager().getTexture(ResourceRegistry.ENTITY_TEXTURE_LOCATION + "shield_base_nopattern").getTexture(), new BufferedImage(64, 64, BufferedImage.TYPE_INT_ARGB));
    }
