...
public class ModelUtils {
    ...
    public static String getNamespace(ICMaterial icMaterial) {
        if (IC.version.isOlderThan(MCVersion.V1_14)) {
    ...
    private static boolean shouldUseLegacyModelName(ICMaterial icMaterial) {
        ...
        if (IC.version.isNewerOrEqualTo(MCVersion.V1_11) && icMaterial.isOneOf(Collections.singletonList("CONTAINS:shulker_box"))) {
            return false;
        }
        return true;
    }
    ...
    public static String getItemModelKey(ICMaterial icMaterial) {
        if (AwesomeChatDiscordAddon.debug) {
