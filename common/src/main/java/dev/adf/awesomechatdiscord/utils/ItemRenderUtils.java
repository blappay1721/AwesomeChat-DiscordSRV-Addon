...
import dev.adf.awesomechatdiscord.graphics.BannerGraphics;
import dev.adf.awesomechatdiscord.graphics.BannerGraphics.BannerAssetResult;
...
import dev.adf.awesomechatdiscord.objectholders.ChargeType;
import dev.adf.awesomechatdiscord.objectholders.CustomModelData;
import dev.adf.awesomechatdiscord.objectholders.ItemDamageInfo;
import dev.adf.awesomechatdiscord.objectholders.MoonPhase;
...
import dev.adf.awesomechatdiscord.resources.definitions.item.ItemModelDefinition;
import dev.adf.awesomechatdiscord.resources.definitions.item.ItemModelDefinitionManager;
...
public class ItemRenderUtils {
    ...
    public static ItemStackProcessResult processItemForRendering(ResourceManager manager, OfflineICPlayer player, ItemStack item, EquipmentSlot slot, ModelDisplay.ModelDisplayPosition displayPosition, boolean is1_8, String language) throws IOException {
        ...
        if (hasItemMeta && itemMeta instanceof PotionMeta) {
            ...
            if (!icMaterial.isMaterial(XMaterial.TIPPED_ARROW) && potiontype != null && IC.version.isOlderThan(MCVersion.V1_19_4)) {
        ...
        if (itemModelDefinitionType.equals(ItemModelDefinition.ItemModelDefinitionType.IC_LEGACY)) {
            ...
            if (item.getType().getMaxDurability() > 0) {
                ...
                int damage = IC.version.isLegacy() ? item.getDurability() : ((Damageable) itemMeta).getDamage();
            ...
            if (icMaterial.isMaterial(XMaterial.CHEST) || icMaterial.isMaterial(XMaterial.TRAPPED_CHEST)) {
                ...
                int durability = item.getType().getMaxDurability() - (IC.version.isLegacy() ? item.getDurability() : ((Damageable) itemMeta).getDamage());
                if (durability <= 1) {
                ...
                if (icplayer != null && icplayer.isLocal()) {
                    ...
                    if (IC.version.isNewerOrEqualTo(MCVersion.V1_16)) {
            ...
            } else if (IC.version.isLegacy() && icMaterial.isOneOf(Collections.singletonList("CONTAINS:bed"))) {
            ...
            } else if (IC.version.isNewerOrEqualTo(MCVersion.V1_9) && icMaterial.isMaterial(XMaterial.ENDER_PEARL)) {
                ICPlayer icplayer = player.getPlayer();
                if (icplayer != null && icplayer.isLocal()) {
                    int cooldown = icplayer.getLocalPlayer().getCooldown(item.getType());
                ...
                if (state instanceof DecoratedPot) {
                    ...
                    for (int i = 0; i < materials.size(); i++) {
                        ...
                        if (IC.version.isNewerOrEqualTo(MCVersion.V1_20_3)) {
            ...
            if (IC.version.isBetweenInclusively(MCVersion.V1_20, MCVersion.V1_21_3) && itemMeta instanceof ArmorMeta) {
        ...
        if (itemModelDefinitionType.equals(ItemModelDefinition.ItemModelDefinitionType.MODEL)) {
            ...
            if (propertyType.equals(ItemModelDefinition.ConditionPropertyType.USING_ITEM)) {
                ...
                if (itemStack.getType().getMaxDurability() > 0) {
                    ...
                    int damage = IC.version.isLegacy() ? itemStack.getDurability() : ((Damageable) itemStack.getItemMeta()).getDamage();
            ...
            if (specialModelType.equals(ItemModelDefinition.SpecialModelType.BANNER)) {
                ...
                if (IC.version.isOlderThan(MCVersion.V1_21_7) && ICMaterial.from(itemStack).isMaterial(XMaterial.PLAYER_HEAD)) {
                    BufferedImage skinImage = manager.getTextureManager().getTexture(ResourceRegistry.DEFAULT_WIDE_SKIN_LOCATION).getTexture();
