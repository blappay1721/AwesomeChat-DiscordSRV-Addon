...
package dev.adf.awesomechatdiscord.wrappers;
...
import dev.adf.awesomechatdiscord.graphics.ImageFrame;
...
public class GraphicsToPacketMapWrapper {
    ...
    public void update() {
        ...
        for (int currentTime = 0; currentTime <= totalTime; currentTime += 50) {
            ...
            if (playbackBar) {
                ...
                g.setColor(AwesomeChatDiscordAddon.plugin.playbackBarEmptyColor);
                g.fillRect(0, 126, 128, 2);
                g.setColor(AwesomeChatDiscordAddon.plugin.playbackBarFilledColor);
    ...
    public void show(Player player) {
        ...
        AwesomeChatDiscordAddon.plugin.imagesViewedCounter.incrementAndGet();
        InboundToGameEvents.MAP_VIEWERS.put(player, this);

        NMS.getInstance().sendFakeMainHandSlot(player, mapItem);
        if (AwesomeChatDiscordAddon.imageFrameHook) {
        ...
        }.runTaskTimer(AwesomeChatDiscordAddon.plugin, 0, 1);
    }
