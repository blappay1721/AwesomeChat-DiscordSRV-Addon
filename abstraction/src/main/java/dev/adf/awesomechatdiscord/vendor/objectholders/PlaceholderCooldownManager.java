...
package dev.adf.awesomechatdiscord.vendor.objectholders;
...
import dev.adf.awesomechatdiscord.vendor.objectholders.CooldownResult.CooldownOutcome;
...
public class PlaceholderCooldownManager {
    ...
    public void reloadPlaceholders() {
        List<ICPlaceholder> placeholderList = ICApi.getICPlaceholderList();
    ...
    public CooldownResult checkMessage(UUID uuid, String message) {
        ...
        for (Entry<UUID, Map<UUID, Long>> entry : placeholderTimestamps.entrySet()) {
            ...
            ICPlaceholder placeholder = IC.placeholderList.get(internalId);
            if (placeholder != null) {
                if (placeholder.getKeyword().matcher(message).find()) {
                    if (first) {
                        first = false;
                        if (IC.universalCooldown > 0) {
                            Long lastUniversal = universalTimestamps.get(uuid);
                            if (lastUniversal != null && now - lastUniversal < IC.universalCooldown) {
                                return new CooldownResult(CooldownOutcome.DENY_UNIVERSAL, now, lastUniversal + IC.universalCooldown, null);
                            }
                        }
    ...
    public void setPlayerUniversalLastTimestamp(UUID uuid, long time) {
        if (IC.bungeecordMode && IC.universalCooldown > 0) {
            try {
    ...
    public void setPlayerPlaceholderLastTimestamp(UUID uuid, ICPlaceholder placeholder, long time) {
        if (IC.bungeecordMode && placeholder.getCooldown() > 0) {
            try {
    ...
    public boolean isPlaceholderOnCooldownAt(UUID uuid, ICPlaceholder placeholder, long time) {
        ...
        if (universalLastTimestamp >= 0 && IC.universalCooldown > 0 && time - universalLastTimestamp < IC.universalCooldown && universalLastTimestamp < time) {
            return true;
        }
