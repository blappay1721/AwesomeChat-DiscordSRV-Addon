...
public class NMS {
    ...
    public synchronized static NMSWrapper getInstance() {
        ...
        try {
            Class<NMSWrapper> nmsImplClass = (Class<NMSWrapper>) Class.forName("dev.adf.awesomechatdiscord.vendor.nms.IC" + IC.version.name());
            instance = nmsImplClass.getConstructor().newInstance();
            NMSWrapper.setup(instance, IC.plugin);
            return instance;
